...
public class Exercise17_14 {
    ...
    public static void main(String[] args) {
        ...
        System.out.print("Input file (decoded):\n[-] "); //make files
        File inputFile = new File(input.nextLine());
        System.out.print("Output file (encoded):\n[-] ");
        File outputFile = new File(input.nextLine());

        String given = ""; //write some to input
        while (!given.equals("EXIT")) {
            System.out.print("Write to input file (EXIT to exit)\n[-] ");
            given = input.nextLine();
            try ( DataOutputStream inputWriter = new DataOutputStream(new FileOutputStream(inputFile, true)); ) {
                if (!given.equals("EXIT"))
                    inputWriter.writeBytes(given);
            } catch (Exception ex) {
                System.exit(13);
            }
        }
